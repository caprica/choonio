...
class ScheduledBackupServiceTest {
    ...
    @Test
    void itExecutesBackupWhenBackupOnScheduleEnabled() {
        ReflectionTestUtils.setField(scheduledBackupService, "enableOnSchedule", true);
        when(backupService.backupData()).thenReturn(Mono.create(MonoSink::success));
        scheduledBackupService.backupData();
        verify(backupService).backupData();
    }

    @Test
    void itSkipsBackupWhenBackupOnScheduleDisabled() {
        ReflectionTestUtils.setField(scheduledBackupService, "enableOnSchedule", false);
        scheduledBackupService.backupData();
