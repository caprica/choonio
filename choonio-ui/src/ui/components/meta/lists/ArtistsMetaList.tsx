...
interface ArtistsMetaListProps {
    artists: Array<string>
}

export default function ArtistsMetaList({ artists }: ArtistsMetaListProps) {
    const { gotoArtist } = useNavigation()
    const handleClickArtist = (artist: string) => () => gotoArtist({ type: MediaType.Artist, artistName: artist })
    return <MetaList items={artists} onClickItem={handleClickArtist} />
}
