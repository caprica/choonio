...
/*!
 * This is a slightly hacked-up version of the original found here:
 *
 * wordcloud2.js
...
;(function (global) {
    ...
    var shuffleArray = function shuffleArray(arr, settings) {
        for (var j, x, i = arr.length; i; j = Math.floor(settings.rng() * i), x = arr[--i], arr[i] = arr[j], arr[j] = x) {}
        return arr
    }
    ...
    var WordCloud = function WordCloud(elements, options) {
        ...
        */
                ...
                case 'star':
                    settings.shape = function shapeStar(theta) {
                        ...
                        if (((theta + 0.955) % ((2 * Math.PI) / 5)) - (2 * Math.PI) / 10 >= 0) {
                            return (
                                1 /
                                (Math.cos((2 * Math.PI) / 10 - thetaPrime) + 3.07768 * Math.sin((2 * Math.PI) / 10 - thetaPrime))
                            )
                        } else {
        ...
        function random_hsl_color(min, max) {
            return (
                'hsl(' +
                (settings.rng() * 360).toFixed() +
                ',' +
                (settings.rng() * 30 + 70).toFixed() +
                '%,' +
                (settings.rng() * (max - min) + min).toFixed() +
                '%)'
            )
        }
        ...
        var getRotateDeg = function getRotateDeg() {
            ...
            if (settings.rng() > settings.rotateRatio) {
                return 0
            }
            ...
            if (rotationSteps > 0) {
                ...
                return minRotation + (Math.floor(settings.rng() * rotationSteps) * rotationRange) / (rotationSteps - 1)
            } else {
                return minRotation + settings.rng() * rotationRange
            }
        }
