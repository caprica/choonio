...
interface AlbumSearchBarOptionProps {
    mediaId: AlbumIdentity
    onClickPlay: () => void
}

export default function AlbumSearchBarOption({ mediaId, onClickPlay }: AlbumSearchBarOptionProps) {
    return (
        <SearchBarOption
            cover={<AlbumCover artistName={mediaId.albumArtistName} albumName={mediaId.albumName} size={ArtSize.Small} />}
            primary={mediaId.albumName}
            secondary={`Album by ${mediaId.albumArtistName}`}
