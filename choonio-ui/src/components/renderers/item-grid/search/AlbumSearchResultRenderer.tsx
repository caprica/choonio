/*
 * This file is part of Choonio.
 *
 * Choonio is free software: you can redistribute it and/or modify it under the
 * terms of the GNU Affero General Public License as published by the Free
 * Software Foundation, either version 3 of the License, or (at your option)
 * any later version.
 *
 * Choonio is distributed in the hope that it will be useful, but WITHOUT ANY
 * WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS
 * FOR A PARTICULAR PURPOSE.  See the GNU Affero General Public License for
 * more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with Choonio.  If not, see <https://www.gnu.org/licenses/>.
 */

import { useAlbumActions } from '../../../../hooks/actions/useAlbumActions'
import { ArtSize } from '../../../../api/model/art-model'
import { useAlbumMenu } from '../../../context/menus/album/AlbumMenuContext'
import AlbumCover from '../../../covers/AlbumCover'
import ItemGridRenderer from '../../../item-grid/renderer/ItemGridRenderer'
import { AlbumIdentity } from '../../../../api/model/identity-model'

interface AlbumSearchResultRendererProps {
    mediaId: AlbumIdentity
    highlight?: string
    onClick: () => void
}

export default function AlbumSearchResultRenderer({ mediaId, highlight, onClick }: AlbumSearchResultRendererProps) {
    const { openAlbumMenu } = useAlbumMenu()

    const { playAlbum } = useAlbumActions()

    const handleClickPlay = () => playAlbum(mediaId)

    const handleClickMenu = (_anchorEl: HTMLElement) => false
    // const handleClickItemMenu = (item: SearchResultData, anchorEl: HTMLElement) => openAlbumMenu(item, anchorEl)

    return (
        <ItemGridRenderer
            art={<AlbumCover artistName={mediaId.albumArtistName} albumName={mediaId.albumName} size={ArtSize.Medium} />}
            primary={mediaId.albumArtistName}
            secondary={mediaId.albumName}
            highlight={highlight}
            onClick={onClick}
            onClickPlay={handleClickPlay}
            onClickMenu={handleClickMenu}
        />
    )
}
