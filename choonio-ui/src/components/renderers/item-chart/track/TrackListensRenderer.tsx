...
import TrackListensMeta from '../../../../ui/pages/stats/meta/TrackListensMeta'
...
export default function TrackListensRenderer({ item, rank, onClickPrimary }: ListensRendererType<TrackPlaysData>) {
    return (
        <ItemChartRenderer
            item={item}
            rank={rank}
            primary={item.mediaId.trackName}
            art={<AlbumCover artistName={item.mediaId.albumArtistName} albumName={item.mediaId.albumName} size={ArtSize.Small} />}
            meta={<TrackListensMeta item={item} />}
